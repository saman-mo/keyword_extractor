...
class Job2JobsService(CachedRequestService):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        """
        Service that fetches similar job titles for a provided job title. The request is cached to improve performance
        for subsequent calls.

        :param endpoint_base_url: base url of the services endpoint, e.g. https://api-dev.services.ai-labs...
        ...
        """
        self._url = f"{endpoint_base_url}/v1/ai/job2jobs"
        self._thresh = thresh
        self._topn = topn +1
        super().__init__(
            backend=backend,
    ...
    def __call__(self, job_title: str, bearer_token: str) -> Dict[dict, float]:
        ...
        response = self._get(url, headers={'Authorization': bearer_token})
    ...
    def _process_response(self, response: Response, original_job_title: str = None) -> dict:
        ...
        suggestions = {k: v for k, v in suggestions.items() if v > self._thresh and k != original_job_title}
        return suggestions
