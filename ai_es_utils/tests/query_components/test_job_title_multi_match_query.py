...
from ai_es_utils.queries.components import JobTitleMultiMatchQuery
...
target_query_body = load_base_query("job_title_multi_match_query.json")
target_query_body_direktor = load_base_query("job_title_query_multi_match_gender_expansion_direktor.json")
...
job_title_query = JobTitleMultiMatchQuery(
...
@pytest.mark.parametrize(
    "input_job_title, expected_query_body", [
        ...
        ("direktor/in", target_query_body_direktor),  # neutral to all
        ("Direktor/in", target_query_body_direktor)  # upper case neutral to all
    ]
)
